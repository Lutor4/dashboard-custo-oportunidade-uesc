...
# UESC | VERSÃO COM MAPA VIA GEOJSON - CORRIGIDA
# ============================================================
#
...
# - geo_municipios_simplificado.geojson  (opcional; pode ser pesado)
#
...
GEOJSON_UF = "geo_uf_simplificado.geojson"
GEOJSON_MICRO = "geo_microrregioes_simplificado.geojson"
GEOJSON_MUN = "geo_municipios_simplificado.geojson"
...
MAPA_REGIAO_UF = {
    ...
    "21": "Nordeste", "22": "Nordeste", "23": "Nordeste", "24": "Nordeste", "25": "Nordeste", "26": "PE", "27": "Nordeste", "28": "Nordeste", "29": "Nordeste",
    "31": "Sudeste", "32": "Sudeste", "33": "Sudeste", "35": "Sudeste",
    "41": "Sul", "42": "Sul", "43": "Sul",
    "50": "Centro-Oeste", "51": "Centro-Oeste", "52": "Centro-Oeste", "53": "Centro-Oeste",
}
MAPA_REGIAO_UF["26"] = "Nordeste"
...
def formatar_colunas_numericas_br(df, casas=0, preservar=None):
    """Formata colunas numéricas para exibição em padrão brasileiro."""
    preservar = set(preservar or [])
    out = df.copy()
    for col in out.columns:
        if col in preservar:
            continue
        if pd.api.types.is_numeric_dtype(out[col]):
            out[col] = out[col].apply(lambda v: formatar_numero_br(v, casas=casas))
    return out
...
def baixar_base_google_drive():
    ...
    if saida.exists() and saida.stat().st_size > 1000:
        return str(saida)
    if gdown is None:
        raise RuntimeError("O pacote gdown não está instalado. Verifique o requirements.txt.")
    url = f"https://drive.google.com/uc?id={GOOGLE_DRIVE_FILE_ID}"
    gdown.download(url, str(saida), quiet=False)
    if not saida.exists() or saida.stat().st_size < 1000:
        raise RuntimeError("Não foi possível baixar a base. Confirme o compartilhamento do Google Drive.")
    return str(saida)
...
def carregar_base(usar_drive=True, arquivo_upload=None):
    if arquivo_upload is not None:
        df = pd.read_csv(arquivo_upload, sep=";", decimal=",", encoding="utf-8-sig", low_memory=False)
    else:
        caminho = baixar_base_google_drive() if usar_drive else ARQUIVO_BASE_LOCAL
        df = pd.read_csv(caminho, sep=";", decimal=",", encoding="utf-8-sig", low_memory=False)
...
@st.cache_data(show_spinner="Carregando GeoJSON...")
...
def tem_geojsons():
    return {
        "UF": Path(GEOJSON_UF).exists(),
        "Microrregião": Path(GEOJSON_MICRO).exists(),
        "Município": Path(GEOJSON_MUN).exists(),
    }


# ============================================================
# ESTRATOS / FILTROS / AGREGAÇÃO
# ============================================================
...
def criar_base_agregada_por_estrato(df_base, grupos, incluir_grupo_geral=True):
    ...
    if incluir_grupo_geral:
        geral = df_base.copy()
        geral["estrato_agregado"] = GRUPO_GERAL
        partes.append(geral)
...
def aplicar_filtros(df_agregado):
    ...
    # Evita duplicação por padrão: usa apenas o grupo geral, quando existir.
    default_estratos = [GRUPO_GERAL] if GRUPO_GERAL in estratos else estratos
    filtro_estrato = st.sidebar.multiselect("Estrato agregado", estratos, default=default_estratos)
...
def mostrar_cabecalho():
    ...
    with st.expander("Metodologia do indicador", expanded=False):
        ...
        st.markdown("Para esta versão do painel, utiliza-se exclusivamente o **custo de oportunidade não negativo**:")
        st.latex(r"""\text{Custo não negativo} = \max(\text{Custo de oportunidade}, 0)""")
...
def diagnostico_custo(df):
    with st.expander("Diagnóstico da variável de custo", expanded=False):
        if df.empty:
            st.warning("Sem dados no filtro atual.")
            return
        zeros = int((df["custo_nao_negativo"] == 0).sum())
        diag = pd.DataFrame([{
            "n": len(df),
            "zeros": zeros,
            "% zeros": zeros / len(df) * 100,
            "mínimo": df["custo_nao_negativo"].min(),
            "média": df["custo_nao_negativo"].mean(),
            "mediana": df["custo_nao_negativo"].median(),
            "máximo": df["custo_nao_negativo"].max(),
        }])
        st.dataframe(formatar_colunas_numericas_br(diag, casas=2), use_container_width=True)
        st.caption("Por padrão, o filtro de estrato usa apenas o grupo geral `5) >= 5 ha`, para evitar duplicação das observações.")
...
def consulta_municipio(df_base):
    ...
    termo = st.selectbox("Pesquisar município", municipios, index=None, placeholder="Digite o nome do município...")
    ...
    por_estrato = (
        dados.groupby("estrato_area", as_index=False)["custo_nao_negativo"]
        .median()
...
def mapa_geojson(df_agg, nivel, limite_superior):
    if len(df_agg) == 0:
        st.warning("Sem dados para gerar mapa.")
        return

    if nivel == "Grande Região":
        st.info("Mapa disponível para Município, Microrregião e UF.")
        return

    if nivel == "Município":
        geo_path = GEOJSON_MUN
        locations = "codigo_municipio"
        featureidkey = "properties.codigo_municipio"
        hover = ["municipio", "uf_sigla", "regiao", "custo_formatado"]
    elif nivel == "Microrregião":
        geo_path = GEOJSON_MICRO
        locations = "codigo_microrregiao"
        featureidkey = "properties.codigo_microrregiao"
        hover = ["microrregiao", "regiao", "custo_formatado"]
    else:
        geo_path = GEOJSON_UF
        locations = "codigo_uf"
        featureidkey = "properties.codigo_uf"
        hover = ["uf_sigla", "regiao", "custo_formatado"]

    geojson = carregar_geojson(geo_path)
    if geojson is None:
        st.warning(
            f"GeoJSON não encontrado: `{geo_path}`. "
            "Gere esse arquivo com o script `gerar_geojsons.py` e envie para o GitHub."
        )
        return

    dados = df_agg.copy()
    dados["custo_formatado"] = dados["custo_agregado"].apply(formatar_numero_br)

    tickvals, ticktext = gerar_ticks_br(limite_superior)
    ...
    fig = px.choropleth_map(
        ...
        zoom=3.4,
        opacity=0.85,
        labels={"custo_agregado": "Custo não negativo"},
    )
    fig.update_layout(
        map_style="white-bg",
...
def grafico_histograma_por_estrato(df):
    ...
    modo = st.radio("Tipo de histograma", ["Um estrato por vez", "Painel com todos os estratos"], horizontal=True)
...
def tabela_dados(df):
    ...
    mostrar_formatado = formatar_colunas_numericas_br(
        mostrar,
        casas=2,
        preservar={"codigo_municipio", "codigo_microrregiao"}
    )

    st.dataframe(mostrar_formatado, use_container_width=True, height=450)
...
with abas[1]:
    st.subheader(f"Mapa por {nivel}")
    st.caption("O mapa usa arquivos GeoJSON simplificados enviados ao GitHub; não usa geobr/geopandas no Streamlit Cloud.")
    status_geo = tem_geojsons()
    st.write(f"GeoJSONs disponíveis — UF: {status_geo['UF']}; Microrregião: {status_geo['Microrregião']}; Município: {status_geo['Município']}")
    mapa_geojson(df_agg, nivel, limite_superior)
with abas[2]:
...
with abas[6]:
    ...
    resumo_estrato = (
        df_filtrado.groupby("estrato_agregado", as_index=False, observed=True)["custo_nao_negativo"]
        .agg(media="mean", mediana="median", minimo="min", maximo="max", n="count")
    )
    resumo_regiao = (
        df_filtrado.groupby("regiao", as_index=False)["custo_nao_negativo"]
        .agg(media="mean", mediana="median", minimo="min", maximo="max", n="count")
    )
    st.write("#### Por estrato agregado")
    st.dataframe(formatar_colunas_numericas_br(resumo_estrato, casas=2), use_container_width=True)
    st.write("#### Por Grande Região")
    st.dataframe(formatar_colunas_numericas_br(resumo_regiao, casas=2), use_container_width=True)
