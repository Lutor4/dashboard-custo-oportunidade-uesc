...
def mapa_folium(df_agg, nivel, limite_superior, renderizar_mapa=True):
    ...
    df_mapa = df_agg.copy()
    # No mapa, usar ponto como separador de milhar e sem casas decimais.
    df_mapa["custo_agregado_formatado"] = df_mapa["custo_agregado"].apply(lambda x: formatar_numero_br(x, casas=0))
    ...
    if nivel == "Município":
        ...
        pares = [("microrregiao", "Microrregião"), ("uf_sigla", "UF"), ("regiao", "Região"), ("custo_agregado_formatado", "Custo não negativo")]
    else:
...
with abas[0]:
    ...
    mapa_folium(df_agg, nivel, limite_superior, renderizar_mapa)
