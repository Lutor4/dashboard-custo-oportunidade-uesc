...
def _listar_texto_itens(itens):
    itens = [str(x).strip() for x in itens if str(x).strip()]
    if not itens:
    ...
    return ", ".join(f'"{x}"' for x in itens[:-1]) + f' e "{itens[-1]}"'
...
def _classificar_assimetria(skew, media=None, mediana=None):
    """Classifica o formato da distribuição e devolve texto acessível."""
    if pd.isna(skew):
        return {
            "rotulo": "formato não definido",
            "explicacao": "não há valores suficientes para avaliar o formato da distribuição com segurança.",
            "decisao": "Use a faixa predominante e o valor representativo como referência inicial, evitando conclusões sobre concentração dos valores.",
        }

    if abs(skew) < 0.25:
        return {
            "rotulo": "relativamente equilibrada",
            "explicacao": "os valores não se concentram claramente apenas nas faixas mais baixas ou mais altas.",
            "decisao": "Isso sugere que o valor representativo resume razoavelmente o comportamento geral do grupo analisado.",
        }
    ...
    if skew > 0:
        forte = skew >= 0.75
        explicacao = (
            "a maior parte dos valores está concentrada em faixas mais baixas, mas existem alguns registros com custos bem superiores aos demais."
            if forte else
            "há uma tendência de concentração em valores mais baixos, com alguns registros acima do comportamento predominante."
        )
        if media_maior:
            explicacao += " Isso também aparece porque a média ficou acima do valor representativo."
        return {
            "rotulo": "concentrada em valores mais baixos, com alguns valores altos",
            "explicacao": explicacao,
            "decisao": "As localidades associadas aos maiores valores merecem atenção específica, pois podem refletir condições produtivas, econômicas ou ambientais diferentes do padrão geral.",
        }

    explicacao = (
        "a maior parte dos valores está concentrada em faixas mais altas, mas existem alguns registros com custos bem inferiores aos demais."
        if skew <= -0.75 else
        "há uma tendência de concentração em valores mais altos, com alguns registros abaixo do comportamento predominante."
    )
    if media_menor:
        explicacao += " Isso também aparece porque a média ficou abaixo do valor representativo."
    return {
        "rotulo": "concentrada em valores mais altos, com alguns valores baixos",
        "explicacao": explicacao,
        "decisao": "As localidades com valores mais baixos podem representar situações diferenciadas e também merecem análise específica.",
    }
...
def _classificar_variacao(cv):
    ...
    if pd.isna(cv):
        return {
            "rotulo": "não avaliada",
            "texto": "não foi possível avaliar a variação dos valores com segurança.",
            "decisao": "A leitura deve se concentrar no valor representativo e na faixa predominante.",
        }
    if cv < 0.15:
        return {
            "rotulo": "baixa",
            "texto": "os valores são relativamente próximos entre si.",
            "decisao": "Isso sugere um comportamento mais homogêneo, em que uma referência geral tende a representar melhor a maior parte das localidades analisadas.",
        }
    if cv < 0.30:
        return {
            "rotulo": "moderada",
            "texto": "existe diferença moderada entre os valores.",
            "decisao": "Há um comportamento predominante, mas algumas localidades podem justificar análises específicas antes da definição de estratégias de uso da terra.",
        }
    return {
        "rotulo": "alta",
        "texto": "os valores variam bastante entre si.",
        "decisao": "Isso indica que políticas ou análises baseadas em um único valor de referência podem mascarar diferenças importantes entre os territórios.",
    }
...
def _coluna_unidade_territorial(df):
    """Identifica a melhor coluna territorial disponível para contar unidades únicas."""
    for col in [
        "codigo_municipio", "municipio", "codigo_microrregiao", "microrregiao",
        "codigo_regiao_intermediaria", "regiao_intermediaria", "regiao_imediata",
        "codigo_uf", "uf_sigla", "regiao"
    ]:
        if col in df.columns:
            return col
    return None
...
def _nome_unidade_por_coluna(col):
    mapa = {
        "codigo_municipio": ("município", "municípios"),
        "municipio": ("município", "municípios"),
        "codigo_microrregiao": ("microrregião", "microrregiões"),
        "microrregiao": ("microrregião", "microrregiões"),
        "codigo_regiao_intermediaria": ("região intermediária", "regiões intermediárias"),
        "regiao_intermediaria": ("região intermediária", "regiões intermediárias"),
        "regiao_imediata": ("região imediata", "regiões imediatas"),
        "codigo_uf": ("unidade da federação", "unidades da federação"),
        "uf_sigla": ("unidade da federação", "unidades da federação"),
        "regiao": ("grande região", "grandes regiões"),
    }
    return mapa.get(col, ("unidade territorial", "unidades territoriais"))


def _resumo_construcao_grupo(dados_grupo, grupo, grupo_coluna):
    """Explica como o grupo do boxplot foi construído."""
    col_unidade = _coluna_unidade_territorial(dados_grupo)
    singular, plural = _nome_unidade_por_coluna(col_unidade)
    n_unidades = dados_grupo[col_unidade].nunique() if col_unidade else np.nan
    n_registros = len(dados_grupo)

    if grupo_coluna == "estrato_agregado" and "estrato_area" in dados_grupo.columns:
        estratos_originais = sorted(dados_grupo["estrato_area"].dropna().astype(str).unique())
        n_estratos = len(estratos_originais)
        texto_estratos = _listar_texto_itens(estratos_originais)

        if col_unidade:
            registros_por_unidade = dados_grupo.groupby(col_unidade, dropna=True).size()
            minimo = int(registros_por_unidade.min()) if len(registros_por_unidade) else 0
            maximo = int(registros_por_unidade.max()) if len(registros_por_unidade) else 0
            media = registros_por_unidade.mean() if len(registros_por_unidade) else np.nan
            unidade_txt = plural if int(n_unidades) != 1 else singular

            if minimo == maximo:
                frase_registros = (
                    f"Cada {singular} contribui com **{minimo} {_plural(minimo, 'registro')}** para este grupo."
                )
            else:
                frase_registros = (
                    f"Cada {singular} contribui com **entre {minimo} e {maximo} registros** para este grupo "
                    f"(média de {formatar_numero_br(media, casas=1)} registros por {singular})."
                )

            return f"""
**Como este grupo foi construído.** O estrato agregado **{grupo}** reúne **{n_estratos} {_plural(n_estratos, 'estrato original', 'estratos originais')}**: {texto_estratos}. Foram considerados **{formatar_numero_br(n_unidades, 0)} {unidade_txt}** e **{formatar_numero_br(n_registros, 0)} registros** de custo de oportunidade. {frase_registros}
""".strip()

        return f"""
**Como este grupo foi construído.** O estrato agregado **{grupo}** reúne **{n_estratos} {_plural(n_estratos, 'estrato original', 'estratos originais')}**: {texto_estratos}. O boxplot resume **{formatar_numero_br(n_registros, 0)} registros** de custo de oportunidade pertencentes a esses estratos.
""".strip()

    if col_unidade:
        unidade_txt = plural if int(n_unidades) != 1 else singular
        return f"""
**Como este grupo foi construído.** Este boxplot resume **{formatar_numero_br(n_registros, 0)} registros** de custo de oportunidade associados a **{formatar_numero_br(n_unidades, 0)} {unidade_txt}** no grupo **{grupo}**.
""".strip()

    return f"""
**Como este grupo foi construído.** Este boxplot resume **{formatar_numero_br(n_registros, 0)} registros** de custo de oportunidade no grupo **{grupo}**.
""".strip()


def _comparacao_media_mediana(est):
    if pd.isna(est["media"]) or pd.isna(est["mediana"]):
        return "Não foi possível comparar média e valor representativo neste grupo."
    if est["mediana"] != 0:
    ...
    if pd.notna(dif_rel) and dif_rel > 0.20:
        return (
            f"A média ({_formatar_moeda_ha(est['media'])}) ficou acima do valor representativo "
            f"({_formatar_moeda_ha(est['mediana'])}). Isso indica que alguns registros com custos elevados aumentam a média do conjunto; por isso, o valor representativo é mais adequado para descrever o comportamento predominante."
        )
    if pd.notna(dif_rel) and dif_rel < -0.20:
        return (
            f"A média ({_formatar_moeda_ha(est['media'])}) ficou abaixo do valor representativo "
            f"({_formatar_moeda_ha(est['mediana'])}). Isso indica que alguns registros com custos baixos reduzem a média do conjunto."
        )
    return (
        f"A média ({_formatar_moeda_ha(est['media'])}) e o valor representativo "
        f"({_formatar_moeda_ha(est['mediana'])}) ficaram próximos, sugerindo menor influência de valores extremos sobre o resultado geral."
    )


def _texto_interpretacao_boxplot(df, grupo_coluna, nome_grupo, coluna_valor="custo_nao_negativo"):
    """Gera leitura do boxplot com foco em apoio à decisão."""
    ...
    blocos = [
        "O boxplot mostra a distribuição do custo de oportunidade considerando o valor mínimo, a faixa central dos dados, o valor mais representativo e o valor máximo. Neste gráfico, as hastes representam os valores mínimo e máximo; a plataforma não separa pontos como outliers."
    ]

    resumo_comparativo = []

    for grupo in grupos:
        dados_grupo = df[df[grupo_coluna].astype(str) == str(grupo)].copy()
        ...
        construcao = _resumo_construcao_grupo(dados_grupo, grupo, grupo_coluna)
        comparacao_media = _comparacao_media_mediana(est)
...
**O que os dados mostram.** O valor mais representativo do custo de oportunidade neste grupo é **{_formatar_moeda_ha(est['mediana'])}**. Isso significa que metade dos registros considerados apresenta valores abaixo desse montante e metade apresenta valores acima.

**Faixa predominante.** A parte central dos valores fica entre **{_formatar_moeda_ha(est['q1'])}** e **{_formatar_moeda_ha(est['q3'])}**. Essa faixa reúne a metade central dos registros usados no boxplot e ajuda a identificar o intervalo mais comum do custo de oportunidade neste grupo.

**Variação entre territórios.** A variação dos valores é **{variacao['rotulo']}**: {variacao['texto']} {variacao['decisao']}

**Formato da distribuição.** A distribuição está **{assim['rotulo']}**. Em termos simples, {assim['explicacao']} {assim['decisao']}

**Média e valor representativo.** {comparacao_media}
"""
        blocos.append(bloco.strip())

    if len(resumo_comparativo) >= 2:
        comp = pd.DataFrame(resumo_comparativo)
        comp = comp.dropna(subset=["mediana"])
        if not comp.empty:
            maior_tipico = comp.loc[comp["mediana"].idxmax()]
            menor_tipico = comp.loc[comp["mediana"].idxmin()]
            maior_var = comp.dropna(subset=["cv"])
            texto_var = ""
            if not maior_var.empty:
                grupo_var = maior_var.loc[maior_var["cv"].idxmax()]
                texto_var = f" O grupo **{grupo_var['grupo']}** apresentou a maior variação relativa dos valores, indicando maior heterogeneidade interna."
            blocos.append(f"""
---

#### Síntese comparativa

Entre os grupos exibidos, **{maior_tipico['grupo']}** apresentou o maior valor representativo do custo de oportunidade (**{_formatar_moeda_ha(maior_tipico['mediana'])}**), enquanto **{menor_tipico['grupo']}** apresentou o menor (**{_formatar_moeda_ha(menor_tipico['mediana'])}**).{texto_var} Para gestores e pesquisadores, essa comparação ajuda a identificar quais grupos de área exigem maior atenção e onde políticas uniformes podem não captar diferenças relevantes entre territórios.
""".strip())
...
def _texto_interpretacao_histograma(df, contexto="recorte selecionado", coluna_valor="custo_nao_negativo"):
    ...
    return f"""
**O que este histograma mostra.** Foram considerados **{formatar_numero_br(est['n'], 0)} registros** de custo de oportunidade no {contexto}. O valor mais representativo é **{_formatar_moeda_ha(est['mediana'])}**.

**Faixa predominante.** A metade central dos valores fica entre **{_formatar_moeda_ha(est['q1'])}** e **{_formatar_moeda_ha(est['q3'])}**. Esse intervalo indica onde se concentra a parte mais comum dos resultados.

**Formato da distribuição.** A distribuição está **{assim['rotulo']}**. Em termos simples, {assim['explicacao']} {assim['decisao']}

**Variação dos valores.** A variação é **{variacao['rotulo']}**: {variacao['texto']} {variacao['decisao']}
