...
def _resumo_cobertura_estrato(df_original_filtrado, df_consolidado_estrato, df_contexto, nivel, estrato):
    ...
    media_registros = n_registros / n_unidades if n_unidades else np.nan
    ...
    if n_estratos > 1:
        construcao = (
            ...
            f'Para cada {unidade["singular"]}, a plataforma calcula um único valor representativo do custo de oportunidade neste grupo, usando a medida escolhida no painel.'
        )
    else:
        construcao = (
            f'O grupo **{estrato}** utiliza o estrato original {lista_estratos}. '
            f'Para cada {unidade["singular"]}, a plataforma considera um único valor representativo do custo de oportunidade.'
        )

    cobertura_txt = (
        f'Foram analisadas **{formatar_numero_br(n_unidades, 0)} {unidade["plural"]}**. '
        f'A base original contribuiu com **{formatar_numero_br(n_registros, 0)} registros** de custo de oportunidade '
        f'para esse grupo'
    )
    if pd.notna(media_registros):
        cobertura_txt += f' — em média, **{formatar_numero_br(media_registros, 1)} registros por {unidade["singular"]}**.'
    else:
        cobertura_txt += "."

    if total_unidades > 0:
        cobertura_txt += (
            f' Em relação ao recorte territorial atual, isso representa **{formatar_numero_br(cobertura * 100, 1)}%** '
            f'{unidade["artigo"]} {unidade["plural"]} de referência.'
        )
        if ausentes > 0:
            cobertura_txt += (
                f' As **{formatar_numero_br(ausentes, 0)} {unidade["plural"]}** não representadas podem não ter registros nos estratos selecionados '
                f'ou podem ter ficado fora dos filtros de valor definidos no painel.'
            )
...
def _texto_interpretacao_boxplot_estrato(df_original, df_consolidado, df_contexto, nivel, medida="median"):
    ...
    blocos = [
        ...
        f"Quando um grupo reúne mais de um estrato original, a plataforma consolida os registros usando a **{criterio}** definida no painel. "
...
def _texto_interpretacao_boxplot_regiao(df_consolidado_regiao, nivel, medida="median"):
    est = _estatisticas_interpretacao(df_consolidado_regiao.rename(columns={"custo_grafico": "custo_nao_negativo"}))
    if est is None:
    ...
    criterio = "média" if medida == "mean" else "mediana"
    return f"""
Cada ponto usado neste boxplot representa um valor consolidado por {unidade['singular']}, calculado pela **{criterio}** dos estratos selecionados no painel.

O valor típico do conjunto é **{_formatar_moeda_ha(est['mediana'])}**, com média de **{_formatar_moeda_ha(est['media'])}**. A metade central {unidade['artigo']} {unidade['plural']} fica entre **{_formatar_moeda_ha(est['q1'])}** e **{_formatar_moeda_ha(est['q3'])}**.

Para tomada de decisão, a variação é **{variacao['rotulo']}**: {variacao['explicacao']} A distribuição está **{assim['rotulo']}**; {assim['explicacao']}
""".strip()
...
def _texto_interpretacao_histograma(df_hist, nivel, contexto="recorte selecionado", coluna_valor="custo_grafico"):
    ...
    return f"""
O histograma mostra quantas {unidade['plural']} estão em cada faixa de custo de oportunidade para **{contexto}**.

Foram consideradas **{formatar_numero_br(est['n'], 0)} {unidade['plural']}**. {media_txt} A maior parte dos valores centrais fica entre **{_formatar_moeda_ha(est['q1'])}** e **{_formatar_moeda_ha(est['q3'])}**.
