...
def criar_legenda_mapa_html(limite_superior):
    """Cria legenda HTML própria, com ponto como separador de milhar."""
    limite = float(limite_superior) if limite_superior is not None else 0.0
    ticks = np.linspace(0, limite, 6)
    labels = [formatar_numero_br(x, casas=0) for x in ticks]

    cores = ["#ffffe5", "#fff7bc", "#fee391", "#fec44f", "#fe9929", "#cc4c02"]
    linhas = "".join(
        f"""
        <div style="display:flex; align-items:center; gap:6px; margin-bottom:3px;">
            <span style="display:inline-block; width:18px; height:12px; background:{cor}; border:1px solid #999;"></span>
            <span>{lab}</span>
        </div>
        """
        for cor, lab in zip(cores, labels)
    )
    ...
    <div style="
        ...
        color: #222;
    ">
        <div style="font-weight: 700; margin-bottom: 6px;">Custo de oportunidade não negativo</div>
        {linhas}
...
def cor_mapa(valor, limite_superior):
    """Retorna uma cor da paleta YlOrBr sem usar legenda automática."""
    try:
        v = float(valor)
    except Exception:
        return "#f0f0f0"
    ...
    limite = float(limite_superior) if limite_superior else 1.0
    if limite <= 0:
        limite = 1.0

    proporcao = max(0.0, min(1.0, v / limite))
    cores = ["#ffffe5", "#fff7bc", "#fee391", "#fec44f", "#fe9929", "#cc4c02"]
    idx = min(int(proporcao * (len(cores) - 1)), len(cores) - 1)
    return cores[idx]


def mapa_folium(df_agg, nivel, limite_superior, renderizar_mapa=True):
    """
    Renderiza o mapa sem tooltip automático do Folium e sem legenda automática.
    Isso elimina o AssertionError e garante ponto como separador de milhar na legenda.
    """
    ...
    if nivel == "Município" and len(df_agg) > 2500:
        st.warning(
            ...
            "Filtre por Região/UF/Microrregião ou use o nível Microrregião/UF para visualizar mais rápido."
        )
        return
    ...
    # Mantém somente as geometrias que têm dados. Assim, quando há filtro territorial,
    # o mapa não mostra o Brasil inteiro em cinza.
    ...
    def style_function(feature):
        ...
        return {
            "fillColor": cor_mapa(valor, limite_superior),
            "color": "white",
            "weight": 0.5,
            "fillOpacity": 0.9,
        }
    ...
    # Sem tooltip/popup automático: evita AssertionError em GeoJSON com propriedades heterogêneas.
    folium.GeoJson(
        mapa,
    ...
    # Legenda manual, em português e com ponto no milhar.
    m.get_root().html.add_child(folium.Element(criar_legenda_mapa_html(limite_superior)))
...
with abas[0]:
    ...
    mapa_folium(df_agg, nivel_painel, limite_superior, renderizar_mapa)

with abas[1]:
    st.subheader(f"Ranking por {nivel_painel}")
    grafico_barras(df_agg, nivel_painel)
