...
def aplicar_filtros(df_agregado):
    ...
    # Primeiro aplicamos apenas os filtros geográficos.
    # Esta base serve como referência de cobertura: quantas unidades territoriais
    # existiam antes dos filtros de estrato e de faixa de custo.
    df = df_agregado.copy()
    ...
    df_contexto_cobertura = df.copy()

    # A partir daqui entram os filtros analíticos que afetam gráficos e mapas.
    df = df[df["custo_nao_negativo"] <= limite_superior].copy()
    ...
    return df, limite_superior, faixas_legenda, df_contexto_cobertura
...
def _nome_unidade_territorial(nivel):
    mapa = {
        "Município": {"singular": "município", "plural": "municípios", "artigo": "dos"},
        "Microrregião": {"singular": "microrregião", "plural": "microrregiões", "artigo": "das"},
        "Região Intermediária": {"singular": "região intermediária", "plural": "regiões intermediárias", "artigo": "das"},
        "Região Imediata": {"singular": "região imediata", "plural": "regiões imediatas", "artigo": "das"},
        "UF": {"singular": "unidade da federação", "plural": "unidades da federação", "artigo": "das"},
        "Grande Região": {"singular": "grande região", "plural": "grandes regiões", "artigo": "das"},
    }
    return mapa.get(nivel, {"singular": "unidade territorial", "plural": "unidades territoriais", "artigo": "das"})


def _colunas_unidade_territorial(df, nivel):
    if nivel == "Município":
        cols = ["codigo_municipio", "municipio", "uf_sigla", "regiao"]
    elif nivel == "Microrregião":
        cols = ["codigo_microrregiao", "microrregiao", "uf_sigla", "regiao"]
    elif nivel == "Região Intermediária":
        cols = ["regiao_intermediaria", "uf_sigla", "regiao"]
        if "codigo_regiao_intermediaria" in df.columns:
            cols.insert(0, "codigo_regiao_intermediaria")
    elif nivel == "Região Imediata":
        cols = ["regiao_imediata", "uf_sigla", "regiao"]
    elif nivel == "UF":
        cols = ["codigo_uf", "uf_sigla", "regiao"]
    elif nivel == "Grande Região":
        cols = ["regiao"]
    else:
        cols = ["municipio"]
...
def _id_unidade_territorial(df, nivel):
    cols_preferidas = {
        "Município": ["codigo_municipio"],
        "Microrregião": ["codigo_microrregiao"],
        "Região Intermediária": ["codigo_regiao_intermediaria", "regiao_intermediaria", "uf_sigla"],
        "Região Imediata": ["regiao_imediata", "uf_sigla"],
        "UF": ["codigo_uf"],
        "Grande Região": ["regiao"],
    }
    cols = [c for c in cols_preferidas.get(nivel, ["municipio"]) if c in df.columns]
    if not cols:
        cols = _colunas_unidade_territorial(df, nivel)[:1]
    if not cols:
...
def _classificar_assimetria(skew, media=None, mediana=None):
    """Classifica o formato da distribuição a partir dos dados, em linguagem simples."""
    if pd.isna(skew):
        return {
            "rotulo": "formato não definido",
            "explicacao": "não há valores suficientes para avaliar o formato da distribuição com segurança.",
        }

    if abs(skew) < 0.25:
        return {
            "rotulo": "mais equilibrada",
            "explicacao": "os valores se distribuem de forma relativamente equilibrada em torno da faixa central.",
        }
    ...
    if skew > 0:
        explicacao = "a maior parte das unidades fica em faixas menores, mas há algumas com valores bem mais altos."
        if media_maior:
            explicacao += " Por isso, a média tende a ficar acima da mediana."
        return {"rotulo": "puxada para valores mais altos", "explicacao": explicacao}

    explicacao = "a maior parte das unidades fica em faixas maiores, mas há algumas com valores bem mais baixos."
    if media_menor:
        explicacao += " Por isso, a média tende a ficar abaixo da mediana."
    return {"rotulo": "puxada para valores mais baixos", "explicacao": explicacao}


def _classificar_variacao(cv):
    """Classifica a variação relativa usando o coeficiente de variação."""
    if pd.isna(cv):
        return {"rotulo": "não avaliada", "explicacao": "não foi possível avaliar a diferença entre as unidades."}
    if cv < 0.15:
        return {"rotulo": "baixa", "explicacao": "os valores são parecidos entre as unidades analisadas."}
    if cv < 0.30:
        return {"rotulo": "moderada", "explicacao": "há diferenças visíveis entre as unidades, mas sem grande afastamento do padrão geral."}
    return {"rotulo": "alta", "explicacao": "há diferenças importantes entre as unidades analisadas."}
...
def _consolidar_para_grafico(df, nivel, medida="median", por_estrato=True):
    """Gera a base usada nos boxplots e histogramas.

    O banco original possui um registro por unidade territorial e estrato original.
    Para que os quartis sejam interpretáveis para gestores, esta função consolida
    esses registros antes do gráfico. Assim, cada município/microrregião/UF etc.
    entra apenas uma vez em cada estrato agregado.
    """
    if df is None or df.empty:
        return pd.DataFrame()

    b = df.copy()
    b["custo_nao_negativo"] = pd.to_numeric(b["custo_nao_negativo"], errors="coerce")
    b = b[b["custo_nao_negativo"].notna()].copy()
    if b.empty:
        return b

    group_cols = _colunas_unidade_territorial(b, nivel)
    if por_estrato and "estrato_agregado" in b.columns:
        group_cols = group_cols + ["estrato_agregado"]
    elif "regiao" in b.columns and "regiao" not in group_cols:
        group_cols = group_cols + ["regiao"]

    agg_fun = "mean" if medida == "mean" else "median"
    out = (
        b.groupby(group_cols, as_index=False, observed=True)
        .agg(
            custo_grafico=("custo_nao_negativo", agg_fun),
            registros_originais=("custo_nao_negativo", "count"),
            estratos_originais_observados=("estrato_area", "nunique"),
        )
    )
    return out


def _resumo_cobertura_estrato(df_original_filtrado, df_consolidado_estrato, df_contexto, nivel, estrato):
    unidade = _nome_unidade_territorial(nivel)

    if df_contexto is None or df_contexto.empty:
        total_unidades = 0
    else:
        total_unidades = _id_unidade_territorial(df_contexto, nivel).replace("", np.nan).dropna().nunique()

    dados_originais_estrato = df_original_filtrado[df_original_filtrado["estrato_agregado"].astype(str) == str(estrato)].copy()
    dados_contexto_estrato = df_contexto[df_contexto["estrato_agregado"].astype(str) == str(estrato)].copy() if df_contexto is not None and "estrato_agregado" in df_contexto.columns else pd.DataFrame()

    estratos_originais = sorted(dados_contexto_estrato["estrato_area"].dropna().astype(str).unique()) if not dados_contexto_estrato.empty else sorted(dados_originais_estrato["estrato_area"].dropna().astype(str).unique())
    n_estratos = len(estratos_originais)
    n_unidades = len(df_consolidado_estrato)
    n_registros = int(dados_originais_estrato["custo_nao_negativo"].notna().sum()) if "custo_nao_negativo" in dados_originais_estrato.columns else 0

    if total_unidades > 0:
        ausentes = max(total_unidades - n_unidades, 0)
        cobertura = n_unidades / total_unidades
    else:
        ausentes = 0
        cobertura = np.nan

    lista_estratos = "; ".join([f'"{e}"' for e in estratos_originais]) if estratos_originais else "estratos selecionados"

    if n_estratos > 1:
        construcao = (
            f'O estrato agregado **{estrato}** reúne **{n_estratos} estratos originais**: {lista_estratos}. '
            f'A base municipal possui até **18 estratos originais de área** por município, ou **19** quando o estrato "Total" também é considerado. '
        ...
        construcao = (
            f'O grupo **{estrato}** utiliza o estrato original {lista_estratos}. '
            f'Para cada {unidade["singular"]}, a plataforma calcula **um único valor** a partir dos municípios pertencentes a essa unidade territorial, usando a medida escolhida no painel.'
        )

    cobertura_txt = (
        f'Foram analisadas **{formatar_numero_br(n_unidades, 0)} {unidade["plural"]}**'
    )

    if total_unidades > 0:
        cobertura_txt += (
            f', o que representa **{formatar_numero_br(cobertura * 100, 1)}%** '
            f'{unidade["artigo"]} {unidade["plural"]} do recorte atual.'
        )
    else:
        cobertura_txt += "."

    cobertura_txt += (
        f' Para chegar a esses valores, a plataforma utilizou **{formatar_numero_br(n_registros, 0)} registros da base municipal** '
        f'e calculou **um valor final por {unidade["singular"]}** para este grupo.'
    )

    if total_unidades > 0 and ausentes > 0:
        cobertura_txt += (
            f' As **{formatar_numero_br(ausentes, 0)} {unidade["plural"]}** não representadas não tiveram valor disponível para esse grupo, '
            f'possivelmente por ausência de registros nos estratos selecionados ou por exclusão nos filtros de valor definidos no painel.'
        )

    return construcao, cobertura_txt

def _texto_decisao(est, unidade):
    variacao = _classificar_variacao(est["cv"])
    assim = _classificar_assimetria(est["assimetria"], est["media"], est["mediana"])

    if variacao["rotulo"] == "alta":
        decisao = (
            f"A variação alta indica que {unidade['plural']} com características territoriais semelhantes podem ter custos de oportunidade bem diferentes. "
            "Para planejamento e políticas públicas, isso sugere cautela no uso de um valor único para todo o recorte analisado."
        )
    elif variacao["rotulo"] == "moderada":
        decisao = (
            f"A variação moderada indica diferenças entre {unidade['plural']}, mas ainda permite identificar uma faixa de referência para comparação."
        )
    else:
        decisao = (
            f"A baixa variação indica que os valores são relativamente próximos entre {unidade['plural']}, o que facilita comparações e uso de referências comuns."
        )

    if assim["rotulo"] == "puxada para valores mais altos":
        decisao += " Como há algumas unidades com valores bem acima do padrão central, essas áreas merecem atenção específica na análise territorial."
    elif assim["rotulo"] == "puxada para valores mais baixos":
        decisao += " Como há algumas unidades com valores bem abaixo do padrão central, também vale observar esses casos separadamente."

    return decisao


def _texto_interpretacao_boxplot_estrato(df_original, df_consolidado, df_contexto, nivel, medida="median"):
    if df_consolidado is None or df_consolidado.empty:
        return "Não há dados suficientes para interpretar o boxplot neste recorte."

    unidade = _nome_unidade_territorial(nivel)
    grupos = [g for g in df_consolidado["estrato_agregado"].dropna().astype(str).unique()]
    ordem = [g for g in DEFAULT_GRUPOS.keys() if g in grupos]
    if "5) >= 5 ha" in grupos and "5) >= 5 ha" not in ordem:
        ordem.append("5) >= 5 ha")
    grupos = ordem + [g for g in sorted(grupos) if g not in ordem]

    criterio = "média" if medida == "mean" else "mediana"
    blocos = [
    ...
    for grupo in grupos:
        dados_grafico = df_consolidado[df_consolidado["estrato_agregado"].astype(str) == str(grupo)].copy()
        est = _estatisticas_interpretacao(dados_grafico.rename(columns={"custo_grafico": "custo_nao_negativo"}))
        if est is None:
            continue

        construcao, cobertura_txt = _resumo_cobertura_estrato(df_original, dados_grafico, df_contexto, nivel, grupo)
        variacao = _classificar_variacao(est["cv"])
        assim = _classificar_assimetria(est["assimetria"], est["media"], est["mediana"])
        decisao = _texto_decisao(est, unidade)
...
def _texto_interpretacao_boxplot_regiao(df_consolidado_regiao, nivel, medida="median"):
    if df_consolidado_regiao is None or df_consolidado_regiao.empty:
        return "Não há dados suficientes para interpretar o boxplot por Grande Região neste recorte."

    unidade = _nome_unidade_territorial(nivel)
    criterio = "média" if medida == "mean" else "mediana"
    df_reg = df_consolidado_regiao.copy()
    if "regiao" not in df_reg.columns:
        return "Não foi possível identificar as grandes regiões neste recorte."

    resumo = []
    for regiao, g in df_reg.groupby("regiao", dropna=False):
        est = _estatisticas_interpretacao(g.rename(columns={"custo_grafico": "custo_nao_negativo"}))
        if est is None:
            continue
        variacao = _classificar_variacao(est["cv"])
        assim = _classificar_assimetria(est["assimetria"], est["media"], est["mediana"])
    ...
    if not resumo:
        return "Não há valores suficientes para interpretar o boxplot por Grande Região neste recorte."

    maior = max(resumo, key=lambda x: x["mediana"])
    menor = min(resumo, key=lambda x: x["mediana"])
    mais_variavel = max(resumo, key=lambda x: -np.inf if pd.isna(x["cv"]) else x["cv"])

    blocos = [
        f"Este boxplot compara as grandes regiões a partir dos valores consolidados por {unidade['singular']}. "
        f"Para cada {unidade['singular']}, a plataforma usa a **{criterio}** dos registros selecionados no painel e, depois, compara esses valores dentro de cada grande região.",
        f"**Destaque geral.** A grande região com maior valor típico foi **{maior['regiao']}** ({_formatar_moeda_ha(maior['mediana'])}). "
        f"A menor mediana foi observada em **{menor['regiao']}** ({_formatar_moeda_ha(menor['mediana'])}). "
        f"A maior variação interna apareceu em **{mais_variavel['regiao']}**, indicando diferenças mais marcantes entre {unidade['plural']} dessa região."
    ]
    ...
    for item in sorted(resumo, key=lambda x: x["regiao"]):
        blocos.append(
            ...
            f"A distribuição está **{item['assim']['rotulo']}**; {item['assim']['explicacao']}"
        )
...
def _texto_interpretacao_histograma(df_hist, nivel, contexto="recorte selecionado", coluna_valor="custo_grafico"):
    est = _estatisticas_interpretacao(df_hist.rename(columns={coluna_valor: "custo_nao_negativo"}))
    if est is None:
    ...
    if pd.notna(est["media"]) and pd.notna(est["mediana"]):
        media_txt = f"A média foi **{_formatar_moeda_ha(est['media'])}** e a mediana foi **{_formatar_moeda_ha(est['mediana'])}**."
    else:
        media_txt = ""

    return f"""
O histograma mostra quantas {unidade['plural']} ficam em cada faixa de custo de oportunidade para **{contexto}**. Cada {unidade['singular']} entra uma única vez no gráfico, após a consolidação feita pela medida escolhida no painel.

Foram consideradas **{formatar_numero_br(est['n'], 0)} {unidade['plural']}**. {media_txt} A faixa central dos valores fica entre **{_formatar_moeda_ha(est['q1'])}** e **{_formatar_moeda_ha(est['q3'])}**.

A distribuição está **{assim['rotulo']}**: {assim['explicacao']} A variação é **{variacao['rotulo']}**, ou seja, {variacao['explicacao']}

Para gestão, essa leitura ajuda a ver se o custo de oportunidade está concentrado em uma faixa comum ou se existem diferenças territoriais que exigem decisões mais específicas.
""".strip()


def grafico_boxplot(df, nivel, medida="median", df_contexto=None):
    if len(df) == 0:
        st.warning("Sem dados para gerar boxplot.")
        return

    df_box_estrato = _consolidar_para_grafico(df, nivel, medida=medida, por_estrato=True)
    df_box_regiao = _consolidar_para_grafico(df, nivel, medida=medida, por_estrato=False)
    ...
    with col1:
        fig = px.box(
            ...
            labels={"estrato_agregado": "Estrato agregado", "custo_grafico": "Custo de oportunidade"},
        )
        fig.update_layout(xaxis_tickangle=-30)
        tickvals, ticktext = gerar_ticks_br(df_box_estrato["custo_grafico"].max())
        fig.update_yaxes(tickvals=tickvals, ticktext=ticktext, title_text="Custo de oportunidade (R$/ha)")
        aplicar_formato_br_plotly(fig)
        st.plotly_chart(fig, use_container_width=True)
        baixar_plotly_png(fig, "boxplot_por_estrato_agregado.png", "boxplot_estrato")
        with st.expander("Leitura automática do boxplot por estrato"):
            st.markdown(_texto_interpretacao_boxplot_estrato(df, df_box_estrato, df_contexto, nivel, medida))
    ...
    with col2:
        fig = px.box(
            ...
            labels={"regiao": "Grande Região", "custo_grafico": "Custo de oportunidade"},
        )
        tickvals, ticktext = gerar_ticks_br(df_box_regiao["custo_grafico"].max())
        fig.update_yaxes(tickvals=tickvals, ticktext=ticktext, title_text="Custo de oportunidade (R$/ha)")
        aplicar_formato_br_plotly(fig)
        st.plotly_chart(fig, use_container_width=True)
        baixar_plotly_png(fig, "boxplot_por_grande_regiao.png", "boxplot_regiao")
        with st.expander("Leitura automática do boxplot por Grande Região"):
            st.markdown(_texto_interpretacao_boxplot_regiao(df_box_regiao, nivel, medida))


def grafico_histograma_por_estrato(df, nivel, medida="median", df_contexto=None):
    if len(df) == 0:
    ...
    df_hist_base = _consolidar_para_grafico(df, nivel, medida=medida, por_estrato=True)
    ...
    if modo == "Um estrato por vez":
        estratos = sorted(df_hist_base["estrato_agregado"].dropna().unique())
        estrato_sel = st.selectbox("Escolha o estrato", estratos)

        dados = df_hist_base[df_hist_base["estrato_agregado"] == estrato_sel].copy()

        fig = px.histogram(
            dados,
            x="custo_grafico",
            nbins=60,
            title=f"Distribuição do custo de oportunidade — {estrato_sel}",
            labels={"custo_grafico": "Custo de oportunidade"},
        )

        max_x = dados["custo_grafico"].max()
        tickvals_x, ticktext_x = gerar_ticks_br(max_x)
        fig.update_xaxes(tickvals=tickvals_x, ticktext=ticktext_x, title_text="Custo de oportunidade (R$/ha)")
        ...
        with st.expander("Leitura automática do histograma"):
            st.markdown(_texto_interpretacao_histograma(dados, nivel, contexto=f"o estrato {estrato_sel}"))
        ...
        fig = px.histogram(
            ...
            facet_col_wrap=2,
            title="Distribuição do custo de oportunidade por estrato agregado",
            labels={"custo_grafico": "Custo de oportunidade"},
        )

        max_x = df_hist_base["custo_grafico"].max()
        tickvals_x, ticktext_x = gerar_ticks_br(max_x)
        fig.update_xaxes(tickvals=tickvals_x, ticktext=ticktext_x, title_text="Custo de oportunidade (R$/ha)")
        fig.update_yaxes(title_text="Frequência")
        ...
        with st.expander("Leitura automática do histograma"):
            st.markdown(_texto_interpretacao_histograma(df_hist_base, nivel, contexto="o conjunto de estratos exibidos"))
...
df_filtrado, limite_superior, faixas_legenda, df_contexto_cobertura = aplicar_filtros(base_agregada)
...
with abas[2]:
    ...
    grafico_boxplot(df_filtrado, nivel_painel, medida_key, df_contexto_cobertura)

with abas[3]:
    grafico_histograma_por_estrato(df_filtrado, nivel_painel, medida_key, df_contexto_cobertura)
