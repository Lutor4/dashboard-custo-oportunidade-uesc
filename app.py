...
ARQUIVO_BASE_PADRAO = "BASEFINAL_mun_sem_menores_5ha_sem_outliers_acima_15000.csv"
...
def carregar_malha_municipal(simplificar=True):
    if geobr is None:
        return None
    gdf = geobr.read_municipality(year=2017)
...
def carregar_malha_uf(simplificar=True):
    if geobr is None:
        return None
    gdf = geobr.read_state(year=2017)
...
def carregar_malha_micro(simplificar=True):
    if geobr is None:
        return None
    gdf = geobr.read_micro_region(year=2017)
...
def mapa_folium(df_agg, nivel, limite_superior, renderizar_mapa=True, forcar_mapa_municipal=False):
    ...
    if nivel == "Município" and len(df_agg) > 2500 and not forcar_mapa_municipal:
        st.warning(
            "O mapa municipal possui muitas geometrias e pode demorar. "
            "Filtre por Região/UF/Microrregião, use o nível Microrregião/UF ou marque a opção para tentar carregar o mapa municipal completo."
        )
        return
    ...
    def style_function(feature):
        valor = feature["properties"].get("custo_agregado", None)
        if valor is None or pd.isna(valor):
            return {"fillColor": "#f0f0f0", "color": "white", "weight": 0.2, "fillOpacity": 0.65}
        return {"fillColor": colormap(float(valor)), "color": "white", "weight": 0.2, "fillOpacity": 0.9}

    if nivel == "Município":
        pares = [("municipio", "Município"), ("uf_sigla", "UF"), ("regiao", "Região"), ("custo_agregado_formatado", "Custo não negativo")]
    elif nivel == "Microrregião":
        pares = [("microrregiao", "Microrregião"), ("regiao", "Região"), ("custo_agregado_formatado", "Custo não negativo")]
    else:
...
def consulta_municipio(df_base):
    ...
    municipio_escolhido = st.selectbox(
        ...
        municipios,
        index=None,
        placeholder="Ex.: Ilhéus, Itabuna, Salvador..."
    )

    if municipio_escolhido is None:
        st.info("Selecione um município para consultar o custo de oportunidade não negativo.")
        return

    dados = df_base[df_base["municipio"] == municipio_escolhido].copy()

    if dados.empty:
        st.warning("Município não encontrado na base.")
        return
    ...
    fig = px.bar(
        ...
        orientation="h",
        text="rotulo",
        title=f"Custo de oportunidade não negativo por estrato — {municipio_escolhido}",
        labels={"mediana_custo": "Mediana do custo", "estrato_area": ""},
    )
    fig.update_traces(textposition="outside")
    tickvals_x, ticktext_x = gerar_ticks_br(por_estrato["mediana_custo"].max())
    fig.update_xaxes(tickvals=tickvals_x, ticktext=ticktext_x)
    fig.update_layout(height=max(450, 30 * len(por_estrato)))
    st.plotly_chart(fig, use_container_width=True)

    cols = [c for c in [
        "codigo_municipio", "municipio", "uf_sigla", "regiao", "microrregiao",
        "estrato_area", "custo_nao_negativo", "numero_estabelecimentos_com_producao",
        "area_estabelecimentos_ha_imputado", "valor_producao_corrigido_mil_reais_imputado",
        "valor_despesa_corrigido_mil_reais_imputado", "resultado_liquido_corrigido_mil_reais_imputado"
    ] if c in dados.columns]
    st.dataframe(dados[cols], use_container_width=True, height=300)
...
with st.sidebar:
    ...
    uploaded = st.file_uploader("Carregar CSV da base", type=["csv"])
    caminho_base = st.text_input("Caminho da base padrão", value=ARQUIVO_BASE_PADRAO)

    st.header("Desempenho")
    renderizar_mapa = st.checkbox("Renderizar mapa interativo", value=True)
    forcar_mapa_municipal = st.checkbox("Tentar carregar mapa municipal completo", value=False)

try:
    if uploaded is not None:
        base_limpa, meta = carregar_base(arquivo_upload=uploaded)
    else:
        base_limpa, meta = carregar_base(caminho_arquivo=caminho_base)
...
with abas[1]:
    ...
    mapa_folium(df_agg, nivel, limite_superior, renderizar_mapa, forcar_mapa_municipal)
