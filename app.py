...
def baixar_plotly_png(fig, nome_arquivo, chave):
    """Cria botão para baixar gráfico Plotly em PNG, quando kaleido estiver disponível."""
    try:
...
def grafico_barras(df_agg, nivel, key_suffix=""):
    ...
    baixar_plotly_png(fig, f"ranking_{nivel}_{key_suffix}.png", f"ranking_{nivel}_{key_suffix}")
...
def mapa_folium(df_agg, nivel, limite_superior, renderizar_mapa=True, faixas_legenda=None):
    ...
    try:
        ...
        st.download_button(
            ...
            mime="text/html",
            key=f"download_mapa_html_{st.session_state['_mapa_download_seq']}",
        )
    except Exception:
        pass
