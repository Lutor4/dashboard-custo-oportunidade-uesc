...
def padronizar_codigo_regiao_intermediaria(s):
    return (
        s.astype(str)
        .str.replace(r"\.0$", "", regex=True)
        .str.replace(r"\D", "", regex=True)
...
def carregar_base(caminho_arquivo=None, arquivo_upload=None):
    ...
    col_regiao_intermediaria = encontrar_coluna(df, [
        "codigo_regiao_intermediaria", "codigo_região_intermediária", "codigo_intermediaria",
        "cod_regiao_intermediaria", "cod_intermediaria", "code_intermediate",
        "regiao_intermediaria_codigo", "região_intermediária_codigo"
    ], obrigatoria=False)
    col_nome_regiao_intermediaria = encontrar_coluna(df, [
        "regiao_intermediaria", "região_intermediária", "nome_regiao_intermediaria", "nome_região_intermediária",
        "regiao_geografica_intermediaria", "região_geográfica_intermediária",
        "regiao_intermediaria_nome", "intermediaria", "regiao_intermed",
        "regiao_intermediaria_ibge", "name_intermediate", "name_intermediate_region"
    ], obrigatoria=False)
    ...
    if col_regiao_intermediaria is not None:
        df["codigo_regiao_intermediaria"] = padronizar_codigo_regiao_intermediaria(df[col_regiao_intermediaria])
    else:
        df["codigo_regiao_intermediaria"] = np.nan

    if col_nome_regiao_intermediaria is not None:
        df["regiao_intermediaria"] = df[col_nome_regiao_intermediaria].astype(str).str.strip()
    else:
        df["regiao_intermediaria"] = df["codigo_regiao_intermediaria"]
...
def aplicar_filtros(df_agregado):
    ...
    if "regiao_intermediaria" in df.columns and df["regiao_intermediaria"].notna().any():
        regioes_intermediarias = sorted([
            ...
            if str(x).strip() and str(x) != "Não informada" and str(x).lower() != "nan"
        ])
...
def agregar(df, nivel, medida="median"):
    if nivel == "Município":
        ...
        group_cols = ["codigo_regiao_intermediaria", "regiao_intermediaria", "uf_sigla", "regiao"]
    elif nivel == "UF":
...
def grafico_barras(df_agg, nivel, key_suffix=""):
    ...
    if nivel == "Município":
        ...
        cat, titulo = "regiao_intermediaria_uf", "Ranking por região intermediária"
    elif nivel == "UF":
    ...
    if nivel == "Microrregião":
        ...
        dados["regiao_intermediaria_uf"] = (
            dados["regiao_intermediaria"].astype(str)
            + " - "
            + dados["uf_sigla"].fillna("").astype(str)
        )
    ...
    try:
        ...
        for i, feat in enumerate(geo.get("features", [])):
            ...
            nome = (
                ...
                or p.get("regiao_intermediaria")
                or p.get("name_intermediate")
                or p.get("name_intermediate_region")
                or p.get("uf_sigla")
                or p.get("regiao")
                or p.get("name_muni")
                or p.get("name_state")
...
def montar_tooltip_feature(props, nivel):
    ...
    if nivel == "Município":
        ...
        titulo = props.get("regiao_intermediaria") or props.get("name_intermediate") or props.get("name_intermediate_region") or "Região Intermediária"
        linhas = [
            ("Região Intermediária", titulo),
            ("UF", props.get("uf_sigla", "")),
            ("Custo de Oportunidade", valor_fmt),
        ]
    elif nivel == "UF":
...
def mapa_folium(df_agg, nivel, limite_superior, renderizar_mapa=True, faixas_legenda=None):
    ...
    if nivel == "Município":
        ...
        if "codigo_regiao_intermediaria" not in df_mapa.columns:
            st.warning("A base filtrada não possui código de Região Intermediária.")
            return

        df_mapa["codigo_regiao_intermediaria"] = padronizar_codigo_regiao_intermediaria(
...
def tabela_dados(df):
    ...
    cols_preferidas = [
        ...
        "codigo_microrregiao", "microrregiao", "codigo_regiao_intermediaria", "regiao_intermediaria", "regiao_imediata",
...
with col_a:
    nivel = st.selectbox("Nível de análise", ["Município", "Microrregião", "Região Intermediária", "UF", "Grande Região"], index=1)
