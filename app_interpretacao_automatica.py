...
def _formatar_moeda_ha(valor):
    """Formata valores do custo de oportunidade em R$/ha."""
    if pd.isna(valor):
        return "não disponível"
    return f"R$ {formatar_numero_br(valor, casas=0)}/ha"


def _classificar_assimetria(skew):
    """Classifica a assimetria de forma simples e vinculada ao coeficiente calculado."""
    if pd.isna(skew):
        return "indefinida", "não foi possível avaliar o formato da distribuição com segurança."

    if abs(skew) < 0.25:
        return (
            "aproximadamente simétrica",
            "os valores estão distribuídos de forma relativamente equilibrada em torno da parte central."
        )
    if skew >= 0.75:
        return (
            "assimétrica à direita",
            "a maior parte dos valores está em faixas menores, enquanto poucos valores mais altos puxam a distribuição para a direita."
        )
    if skew > 0:
        return (
            "levemente assimétrica à direita",
            "há uma tendência de concentração em valores menores, com alguns valores mais altos."
        )
    if skew <= -0.75:
        return (
            "assimétrica à esquerda",
            "a maior parte dos valores está em faixas maiores, enquanto poucos valores mais baixos puxam a distribuição para a esquerda."
        )
    return (
        "levemente assimétrica à esquerda",
        "há uma tendência de concentração em valores maiores, com alguns valores mais baixos."
    )


def _classificar_variacao(cv):
    """Classifica a variação relativa a partir do coeficiente de variação."""
    if pd.isna(cv):
        return "não foi possível avaliar a variação relativa dos valores."
    if cv < 0.15:
        return "a variação entre as observações é baixa."
    if cv < 0.30:
        return "a variação entre as observações é moderada."
    return "a variação entre as observações é alta."
...
def _texto_interpretacao_boxplot(df, grupo_coluna, nome_grupo, coluna_valor="custo_nao_negativo"):
    """Gera texto acessível para boxplot, usando mínimo, Q1, mediana, Q3 e máximo.

    Observação: o boxplot da plataforma é interpretado sem critério de outliers.
    As hastes são descritas como mínimo e máximo dos dados apresentados.
    """
    ...
    # Preserva uma ordem mais natural quando os estratos agregados existem.
    ...
    for grupo in grupos:
        ...
        tipo_assimetria, explicacao_assimetria = _classificar_assimetria(est["assimetria"])
        texto_variacao = _classificar_variacao(est["cv"])

        partes.append(
            f"**{grupo}**: foram analisadas **{formatar_numero_br(est['n'], 0)} observações**. "
            f"O menor valor observado foi **{_formatar_moeda_ha(est['minimo'])}** e o maior foi **{_formatar_moeda_ha(est['maximo'])}**. "
            f"A mediana foi **{_formatar_moeda_ha(est['mediana'])}**, ou seja, metade das observações apresentou valores até esse patamar. "
            f"Cerca de 25% das observações ficaram até **{_formatar_moeda_ha(est['q1'])}**, enquanto 25% ficaram acima de **{_formatar_moeda_ha(est['q3'])}**. "
            f"Os 50% centrais dos valores estão entre **{_formatar_moeda_ha(est['q1'])}** e **{_formatar_moeda_ha(est['q3'])}**. "
            f"Pelo formato dos dados, a distribuição é **{tipo_assimetria}**; isso significa que {explicacao_assimetria} "
            f"Considerando a média e o desvio-padrão, {texto_variacao}"
        )
    ...
    abertura = (
        f"A leitura abaixo usa os mesmos elementos do boxplot apresentado: mínimo, primeiro quartil, mediana, "
        f"terceiro quartil e máximo. A plataforma não classifica pontos como outliers nesse gráfico; portanto, "
        f"a interpretação considera a amplitude completa dos valores exibidos."
    )
    return abertura + "\n\n" + "\n\n".join(partes)
...
def _texto_interpretacao_histograma(df, contexto="recorte selecionado", coluna_valor="custo_nao_negativo"):
    ...
    relacao_media_mediana = ""
    if pd.notna(est["media"]) and pd.notna(est["mediana"]) and est["mediana"] != 0:
        dif_rel = (est["media"] - est["mediana"]) / abs(est["mediana"])
        if dif_rel > 0.20:
            relacao_media_mediana = (
                f" A média (**{_formatar_moeda_ha(est['media'])}**) ficou acima da mediana "
                f"(**{_formatar_moeda_ha(est['mediana'])}**), o que é compatível com a presença de alguns valores mais altos."
            )
        elif dif_rel < -0.20:
            relacao_media_mediana = (
                f" A média (**{_formatar_moeda_ha(est['media'])}**) ficou abaixo da mediana "
                f"(**{_formatar_moeda_ha(est['mediana'])}**), o que é compatível com a presença de alguns valores mais baixos."
            )
        else:
            relacao_media_mediana = (
                f" A média (**{_formatar_moeda_ha(est['media'])}**) e a mediana "
                f"(**{_formatar_moeda_ha(est['mediana'])}**) ficaram próximas, sugerindo menor influência de valores extremos sobre a média."
            )

    return (
        f"No **{contexto}**, o histograma resume **{formatar_numero_br(est['n'], 0)} observações**. "
        f"A mediana foi **{_formatar_moeda_ha(est['mediana'])}**. "
        f"Cerca de 25% das observações ficaram até **{_formatar_moeda_ha(est['q1'])}** e 75% ficaram até **{_formatar_moeda_ha(est['q3'])}**. "
        f"A distribuição é **{tipo_assimetria}**; em termos práticos, {explicacao_assimetria}"
        f"{relacao_media_mediana} "
        f"Pela relação entre média e desvio-padrão, {texto_variacao}"
    )
